const notesContainer = document.getElementById("app");
const addNoteButton = document.createElement("button");

addNoteButton.classList.add("add-note");
addNoteButton.textContent = "+";
notesContainer.appendChild(addNoteButton);

getNotes().forEach((note) => {
  const noteElement = createNoteElement(note.id, note.content);
  notesContainer.insertBefore(noteElement, addNoteButton);
});

addNoteButton.addEventListener("click", () => addNote());

function getNotes() {
  return JSON.parse(localStorage.getItem("stickynotes-notes") || "[]");
}

function saveNotes(notes) {
  localStorage.setItem("stickynotes-notes", JSON.stringify(notes));
}

function createNoteElement(id, content) {
  const element = document.createElement("textarea");

  // Add the note-toggle span
  const noteToggle = document.createElement("span");
  noteToggle.classList.add("note-toggle");
  noteToggle.textContent = "+";
  element.appendChild(noteToggle);

  element.classList.add("note");
  element.value = content;
  element.placeholder = "Empty Sticky Note";

  element.addEventListener("change", () => {
    updateNote(id, element.value);
  });

  element.addEventListener("mouseenter", () => {
    element.classList.add("note-hover");
    noteToggle.textContent = "-";
  });

  element.addEventListener("mouseleave", () => {
    element.classList.remove("note-hover");
    noteToggle.textContent = "+";
  });

  // Add the right-click event listener for deleting the note
  element.addEventListener("contextmenu", (event) => {
    event.preventDefault();
    const doDelete = confirm(
      "Are you sure you wish to delete this sticky note?"
    );

    if (doDelete) {
      deleteNote(id, element);
    }
  });

  return element;
}

function addNote() {
  const notes = getNotes();
  const noteObject = {
    id: Math.floor(Math.random() * 100000),
    content: "",
  };

  const noteElement = createNoteElement(noteObject.id, noteObject.content);
  notesContainer.insertBefore(noteElement, addNoteButton);

  notes.push(noteObject);
  saveNotes(notes);
}

function updateNote(id, newContent) {
  const notes = getNotes();
  const targetNote = notes.filter((note) => note.id == id)[0];

  targetNote.content = newContent;
  saveNotes(notes);
}

function deleteNote(id, element) {
  const notes = getNotes().filter((note) => note.id != id);

  saveNotes(notes);
  notesContainer.removeChild(element);
}
